...
interface
uses
  Windows, Contnrs, BaseClass, DirectShow9, DSUtils;
type
  //
...
begin
  ...
  FreeAndNil(FOnWork);    //@@@ SZ added
  FreeAndNil(FOnDone);    //@@@ SZ added
  FreeAndNil(FOnAllDone); //@@@ SZ added
  FreeAndNil(FOnStop);    //@@@ SZ added

  // empty the done list
  FListDone.Clear;
  FListDone.Free;
  FListDone := nil;
  FreeAndNil(FListWork); //@@@ SZ added
  FreeAndNil(FCSLists);  //@@@ SZ added
  FreeAndNil(FReader);   //@@@ SZ added
end;
