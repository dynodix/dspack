...
interface
uses
  BaseClass, DirectShow9, DSUtils, ActiveX, Windows, SysUtils,
  UAsyncIo;
...
type
  ...
  public
    ...
    constructor Create(Name: String; Unk: IUnknown; const clsid: TGUID;
...
constructor TBCAsyncReader.Create(Name: String; Unk: IUnknown; const clsid: TGUID;
  Stream: TBCAsyncStream; out hr: HResult);
begin
  FCSFilter := TBCCritSec.Create;
  ZeroMemory(@Fmt, SizeOf(TAMMediaType));

  Inherited Create(Name, Unk, FCSFilter, clsid, hr);
...
begin
  FreeAndNil(FOutputPin); //@@@ SZ added
  FreeAndNil(FIO);        //@@@ SZ added
